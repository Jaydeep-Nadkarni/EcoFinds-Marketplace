...
function Navbar({ onSearch, onFilter }) {
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around py-2 shadow-lg z-40">
        {navItems.map(({ path, label, icon: Icon }) => (
          <Link
            key={path}
            to={path}
            className={`flex flex-col items-center p-2 rounded-lg text-xs min-w-[60px] transition-colors
              ${isActive(path) 
                ? 'text-green-600 bg-green-50' 
